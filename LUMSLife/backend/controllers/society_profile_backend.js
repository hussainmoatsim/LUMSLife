...
app.get('/api/profile/edit', function(req, res) {
  ...
  const sql = 'UPDATE Society SET name = ?, email = ?, password_hash = ? WHERE Society_id = ?';
...
app.get('/api/profile/society-members', function(req, res) {
  ...
  const sql = 'SELECT U.name FROM User U JOIN Society_member SM ON U.User_id = SM.User_id JOIN Society_membership SM2 ON SM.member_id = SM2.member_id WHERE SM2.joined = true AND SM2.Society_id = <society_id>;'
...
app.get('/api/profile/posts', function(req, res) {
    ...
    const sql = 'SELECT P.* FROM Society S JOIN Posts P ON S.posts_id = P.posts_id WHERE S.Society_id = <society_id>;';
    const values = [req.query.id];
    db.query(sql, values, function(err, result) {
      if (err) {
        throw err;
...
app.get('/api/profile/view-applicants', function(req, res) {
  ...
  const sql = 'SELECT U.name, S.cv, S.about_me FROM User U JOIN Student S ON U.User_id = S.User_id JOIN Society_member SM ON U.User_id = SM.User_id JOIN Society_membership SM2 ON SM.member_id = SM2.member_id WHERE SM2.joined = false'
  const values = [req.query.id];
  db.query(sql, values, function(err, result) {
    if (err) {
      throw err;
